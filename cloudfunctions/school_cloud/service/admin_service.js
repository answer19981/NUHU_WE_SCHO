...
class AdminService extends BaseCCMiniService {
	...
	async delUser(id) {
		...
		let infoList = await InfoModel.getAll(whereInfo, 'INFO_PIC');
		for (let k in infoList) {
			if (infoList[k].INFO_PIC && infoList[k].INFO_PIC.length > 0) {
				let cloudIds = ccminiStrUtil.getArrByKey(infoList[k].INFO_PIC, 'cloudId');
				ccminiCloudUtil.deleteFiles(cloudIds);
			}
		}
		InfoModel.del(whereInfo);
